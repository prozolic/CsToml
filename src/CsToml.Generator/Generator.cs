...
""");
        ...
        var source = context.SyntaxProvider.ForAttributeWithMetadataName(
            ...
            static (context, token) =>
            {
                ...
                return (typeMeta, constructorMeta);
            });

        context.RegisterSourceOutput(source, EmitForTomlSerializedObject);

        // Filter to an equatable model before collecting, so that the shared key cache is
        // regenerated only when the set of bare keys itself changes.
        var bareKeys = source
            .Select(static (source, token) => CollectPushBareKeys(source.Item1))
            .Collect()
            .Select(static (keys, token) =>
            {
                var definedCacheKeys = new SortedSet<string>(StringComparer.Ordinal);
                foreach (var perType in keys)
                {
                    foreach (var key in perType)
                    {
                        definedCacheKeys.Add(key);
                    }
                }

                return new EquatableArray<string>(definedCacheKeys.ToImmutableArray());
            });

        context.RegisterSourceOutput(bareKeys, EmitForCsTomlKeyCache);
    }

    private void EmitForCsTomlKeyCache(SourceProductionContext context, EquatableArray<string> bareKeys)
    {
        var builder = new StringBuilder();
        foreach (var key in bareKeys)
        {
            builder.AppendLine($"    internal static readonly ImmutableArray<byte> {GetPushBareKeyFieldName(key)} = ImmutableArray.Create(\"{key}\"u8);");
        }
...
""";
        ...
        context.AddSource($"CsTomlKeyCache_generated.g.cs", code);
    }

    private void EmitForTomlSerializedObject(SourceProductionContext context, (TypeMeta, ConstructorMeta) source)
    {
        var (typeMeta, constructorMeta) = source;
...
""");
    ...
    private void GenerateSerializePrimitivePart(StringBuilder builder, TomlValueOnSerializedData tomlValueOnSerializedData, string indent)
    {
        ...
        var writeKeyMethod = GetWriteKeyMethod(accessName);

        builder.AppendLine($$"""
        {{indent}}{{writeKeyMethod}};
...
""");
    ...
    private void GenerateSerializeTomlSerializedObjectPart(StringBuilder builder, TomlValueOnSerializedData tomlValueOnSerializedData, string indent, bool multiple)
    {
        var propertyName = tomlValueOnSerializedData.DefinedName;
        var accessName = tomlValueOnSerializedData.CanAliasName ? tomlValueOnSerializedData.AliasName : propertyName;
        var pushKeyMethod = GetPushKeyMethod(accessName);
        var tableHeaderMethod = GetWriteTableHeaderMethod(accessName);
...
""");
    ...
    private void GenerateSerializeNullableStructWithTypeParameterPart(StringBuilder builder, TomlValueOnSerializedData tomlValueOnSerializedData, string indent)
    {
        ...
        var accessName = tomlValueOnSerializedData.CanAliasName ? tomlValueOnSerializedData.AliasName : propertyName;
        var pushKeyMethod = GetPushKeyMethod(accessName);
        var writeTableHeaderMethod = GetWriteTableHeaderMethod(accessName);
        var symbol = tomlValueOnSerializedData.Symbol;
...
""");
    ...
    private static string GetPushKeyMethod(string? accessName)
        => IsBareKey(accessName)
            ? $"writer.PushBareKey(CsTomlKeyCache.{GetPushBareKeyFieldName(accessName!)}, skipValidation: true)"
            : $"writer.PushKey(@\"{accessName}\"u8)";

    private static string GetWriteKeyMethod(string? accessName)
        => IsBareKey(accessName)
            ? $"writer.WriteBareKey(@\"{accessName}\"u8, skipValidation: true)"
            : $"writer.WriteKey(@\"{accessName}\"u8)";

    private static string GetWriteTableHeaderMethod(string? accessName)
        => IsBareKey(accessName)
            ? $"writer.WriteBareTableHeader(@\"{accessName}\"u8, skipValidation: true)"
            : $"writer.WriteTableHeader(@\"{accessName}\"u8)";

    private static bool IsBareKey(string? accessName)
        => accessName is not null && TomlKeyTypeMatcher.IsMatchBarekey(accessName);

    private static string GetPushBareKeyFieldName(string key)
    {
        // The field name is derived from the key itself, since CsTomlKeyCache is shared by every type.
        // '_' and '-' are escaped so that different keys never map to the same field name.

        var builder = new StringBuilder("__generated__", key.Length + 16);
        foreach (var c in key)
        {
            ...
            {
                builder.Append("_x").Append(((int)c).ToString("X4"));
            }
        }
    ...
    private static EquatableArray<string> CollectPushBareKeys(TypeMeta typeMeta)
    {
        var keys = ImmutableArray.CreateBuilder<string>();

        foreach (var member in typeMeta.OrderedMembers)
        {
            // Only the serialization kinds that emit writer.PushBareKey/PushKey calls.
            switch (member.SerializationKind)
            {
                case TomlSerializationKind.TomlSerializedObject:
                case TomlSerializationKind.Dictionary:
                case TomlSerializationKind.TypeParameter:
                    break;
                case TomlSerializationKind.NullableStructWithTypeParameter:
                    if (member.Symbol.Type is not INamedTypeSymbol)
                        continue;
                    break;
                default:
                    continue;
            ...
            var accessName = member.CanAliasName ? member.AliasName : member.DefinedName;
            if (accessName is null || !TomlKeyTypeMatcher.IsMatchBarekey(accessName))
            {
                continue;
            }
        ...
        return keys.Count == 0 ? EquatableArray<string>.Empty : new EquatableArray<string>(keys.ToImmutable());
    }
