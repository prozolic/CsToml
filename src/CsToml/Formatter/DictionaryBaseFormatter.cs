...
public abstract class DictionaryBaseFormatter<TKey, TValue, TDictionary, TMediator> : ITomlValueFormatter<TDictionary>
    where TDictionary : IEnumerable<KeyValuePair<TKey, TValue>>
{
    public TDictionary Deserialize(ref TomlDocumentNode rootNode, CsTomlSerializerOptions options)
    {
        if (rootNode.NodeCount == 0)
        {
            if (rootNode.Value is TomlTable table)
            {
                ...
                return options.Resolver.GetFormatter<TDictionary>()!.Deserialize(ref tableDocumentNode, options);
            }
    ...
    public void Serialize<TBufferWriter>(ref Utf8TomlDocumentWriter<TBufferWriter> writer, TDictionary target, CsTomlSerializerOptions options)
        where TBufferWriter : IBufferWriter<byte>
    {
        if (target == null)
        {
            ExceptionHelper.ThrowSerializationFailed(typeof(TDictionary));
            return;
        }
    ...
    protected abstract TDictionary Complete(TMediator dictionary);
